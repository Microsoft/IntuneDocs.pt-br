...
description: Saiba como encapsular os aplicativos iOS sem alterar o código do aplicativo em si. Prepare os aplicativos para que você possa aplicar políticas de gerenciamento de aplicativo móvel.
keywords: ''
author: Erikre
ms.author: erikre
manager: dougeby
...
A ferramenta é um aplicativo de linha de comando do macOS que cria um wrapper em torno de um aplicativo. Quando um aplicativo é processado, é possível alterar sua funcionalidade implantando [políticas de proteção de aplicativo](../apps/app-protection-policies.md) nele.
...
7. Após a verificação, clique em **Concordo com a Licença**.

8. Depois de concordar com a licença, conclua com a **compra e ativação do programa**.

9. Se você for um agente de equipe (a pessoa que participa do Programa Corporativo do Desenvolvedor da Apple em nome de sua organização), crie sua equipe primeiro convidando os membros da equipe e atribuindo funções. Para saber como gerenciar sua equipe, leia a documentação da Apple [Managing Your Developer Account Team](https://developer.apple.com/library/content/documentation/IDEs/Conceptual/AppDistributionGuide/ManagingYourTeam/ManagingYourTeam.html#//apple_ref/doc/uid/TP40012582-CH16-SW1) (Gerenciando sua equipe de conta de desenvolvedor).
...
4. Clique em **Certificados, IDs e Perfis**.
   ...
   ![Portal do desenvolvedor da Apple - Certificados, IDs e Perfis](./media/app-wrapper-prepare-ios/iOS-signing-cert-1.png)

5. Clique no botão ![Sinal de adição do portal do Desenvolvedor da Apple](./media/app-wrapper-prepare-ios/iOS-signing-cert-2.png) no canto superior direito para adicionar um certificado do iOS.
...
8. Leia as instruções sobre como criar uma **CSR (Solicitação de Assinatura de Certificado)** usando o aplicativo Acesso ao Conjunto de Chaves no computador macOS.
   ...
   ![Ler as instruções para criar uma CSR](./media/app-wrapper-prepare-ios/iOS-signing-cert-4.png)
...
10. No menu do macOS na parte superior da tela, acesse **Acesso ao Conjunto de Chaves > Assistente de Certificado > Solicitar um Certificado de uma Autoridade de Certificação**.  
    ...
    ![Solicitar um certificado de uma Autoridade de Certificação no Acesso ao Conjunto de Chaves](./media/app-wrapper-prepare-ios/iOS-signing-cert-5.png)
...
13. A Apple gera o certificado de autenticação. Baixe e salve-o em um local fácil de ser lembrado no computador macOS.
    ...
    ![Baixar o certificado de autenticação](./media/app-wrapper-prepare-ios/iOS-signing-cert-7.png)
...
15. Abra **Acesso ao Conjunto de Chaves** novamente. Localize o certificado pesquisando pelo nome na barra de pesquisa na parte superior direita. Clique com o botão direito do mouse no item para exibir o menu e clique em **Obter Informações**. Nas telas de exemplo, estamos usando um certificado de desenvolvimento em vez de um certificado de produção.
    ...
    ![Adicionar o certificado a um conjunto de chaves](./media/app-wrapper-prepare-ios/iOS-signing-cert-8.png)
...
16. Uma janela informativa é exibida. Role até a parte inferior e procure o rótulo **Impressões Digitais**. Copie a cadeia de caracteres **SHA1** (escurecida) para ser usada como o argumento “-c” para a Ferramenta de Disposição do Aplicativo.
    ...
    ![Informações do iPhone - cadeia de caracteres de impressões digitais SHA1](./media/app-wrapper-prepare-ios/iOS-signing-cert-9.png)
...
3. Clique no botão ![Sinal de adição do portal do Desenvolvedor da Apple](./media/app-wrapper-prepare-ios/iOS-signing-cert-2.png) no canto superior direito para adicionar um perfil de provisionamento do iOS.

4. Opte por criar um perfil de provisionamento **Interno** em **Distribuição**.

   ![Selecionar o perfil de provisionamento Interno](./media/app-wrapper-prepare-ios/iOS-provisioning-profile-1.png)
...
1. Habilite as funcionalidades em seu aplicativo:
    ...
    c.  Tome nota de quaisquer IDs que você criar durante o processo. Elas também podem ser chamadas de valores `AppIdentifierPrefix`.
...
2. Habilite os direitos no seu perfil de provisionamento:
    ...
    c.  No seu perfil de provisionamento, habilite os mesmo direitos que você tem em seu aplicativo. Você precisará fornecer as mesmas IDs (os valores `AppIdentifierPrefix`) especificadas durante o desenvolvimento do aplicativo. 
...
Para usar o sinalizador `-citrix`, você também precisará instalar o [wrapper do aplicativo Citrix MDX para iOS](https://docs.citrix.com/en-us/mdx-toolkit/10/xmob-mdx-kit-app-wrap-ios.html) no mesmo computador macOS. Os downloads são encontrados em [Downloads do Citrix XenMobile](https://www.citrix.com/downloads/xenmobile/) e são restritos aos clientes Citrix somente após o login. Certifique-se de que isso esteja instalado no local padrão: `/Applications/Citrix/MDXToolkit`. 

> [!NOTE] 
> O suporte para integração com o Intune e Citrix é limitado apenas a dispositivos iOS 10+.

### <a name="use-the--citrix-flag"></a>Usar o sinalizador `-citrix`

Basta executar o comando geral de agrupamento de aplicativos com o sinalizador `-citrix` anexado. O sinalizador `-citrix` atualmente não aceita argumentos.
