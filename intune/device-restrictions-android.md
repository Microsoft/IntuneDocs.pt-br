...
- Se o aplicativo Portal da Empresa estiver executando uma versão anterior à 1704, nenhuma política de PIN será aplicada ao dispositivo e será exibido um erro no portal do Azure.
- Se o aplicativo do Portal da Empresa executar a versão 1704 ou posterior, apenas um PIN simples poderá ser aplicado. Versões do Android anteriores à 5.0 não oferecem suporte a essa configuração. Nenhum erro é mostrado no console do portal do Azure.
...
- **Aplicativos aprovados**: uma lista de aplicativos que os usuários têm permissão de instalar. Para permanecer em conformidade, os usuários não devem instalar outros aplicativos. Aplicativos que são gerenciados pelo Intune são permitidos automaticamente.
...
- **Adicionar aplicativos por nome de pacote**: usado principalmente para a aplicativos de linha de negócios. Insira o nome do aplicativo e o nome do pacote do aplicativo.
- **Adicionar aplicativos por URL**: insira o nome do aplicativo e sua URL na Google Play Store.
- **Adicionar o aplicativo da Store**: selecione um aplicativo da lista existente de aplicativos gerenciados no Intune.
...
- Adicione aplicativos que você deseja executar quando o dispositivo estiver no modo de quiosque. No modo de quiosque, somente os aplicativos que você adicionar são executados. Os navegadores pré-instalados não são executados como um aplicativo quando o dispositivo estiver no modo de quiosque. Se um navegador for necessário, use o [Managed Browser](app-configuration-managed-browser.md).
...
- **Botão para colocar a tela em modo de suspensão**: escolha **Bloquear** para evitar ou ocultar o botão para colocar a tela em suspensão. **Não configurado** habilita ou desabilita o botão de ativação e suspensão da tela no dispositivo.
...
Também é possível criar perfis de quiosque para dispositivos com [Android Enterprise](device-restrictions-android-for-work.md#kiosk-settings) e [Windows 10](kiosk-settings.md).