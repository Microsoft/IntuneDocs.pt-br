...
- **Regras da NRPT (tabela) de diretiva de resolução de nome**: regras NRPT (tabela) de política de resolução de nome definem como o DNS resolve nomes quando conectados à VPN. Depois que a conexão VPN for estabelecida, escolha quais servidores DNS serão usados pela conexão VPN.
