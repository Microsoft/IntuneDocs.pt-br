...
description: Examine os padrões e as configurações disponíveis encontradas na linha de base de segurança do MDM do Windows para dispositivos Windows 10 que você gerencia com o Intune.
author: brenduns
ms.author: brenduns
manager: dougeby
ms.date: 02/03/2020
...
ms.locfileid: "76977828"
---
# <a name="mdm-security-baseline-settings-for-intune"></a>Configurações de linha de base de segurança MDM para o Intune  

Exiba as configurações de linha de base de segurança do MDM compatíveis com o Microsoft Intune para dispositivos que executam o Windows 10 ou posterior. Os valores padrão para as configurações nessa linha de base representam a configuração recomendada para os dispositivos aplicáveis e podem não ser iguais aos padrões de linha de base de outras linhas de base de segurança.  

A versão de linha de base mais recente é a **Linha de Base de Segurança do MDM para maio de 2019**  

Para saber mais sobre o que mudou na versão mais recente dessa linha de base, confira [O que mudou no novo modelo](#whats-changed-in-the-new-template).  

> [!NOTE]  
> Em junho de 2019, a versão prévia da linha de base de segurança do MDM foi substituída pelo lançamento do modelo *Linha de Base de Segurança do MDM para maio de 2019*, que está em disponibilidade geral (não em versão prévia). Os perfis criados antes da disponibilidade da *Linha de Base de Segurança do MDM para maio de 2019* não serão atualizados para refletir as configurações e os valores que estão na versão da Linha de Base de Segurança do MDM para maio de 2019.  Embora não seja possível criar perfis com base no modelo de versão prévia, você pode editar e continuar a usar perfis criados anteriormente com base no modelo de versão prévia. 
...
- **Bloquear o controle do usuário sobre as instalações**  
  Essa configuração de política permite aos usuários alterar as opções de instalação que normalmente estão disponíveis somente a administradores do sistema. Se você habilitar essa configuração de política, alguns recursos de segurança do Windows Installer serão ignorados. Isso permite a conclusão de instalações que de outra forma seriam interrompidas devido à violação de segurança. Se você desabilitar ou não definir essa configuração de política, os recursos de segurança do Windows Installer evitarão que os usuários alterem as opções de instalação normalmente reservadas a administradores do sistema, como especificar o diretório de instalação de arquivos. Se o Windows Installer detectar que um pacote de instalação permitiu que o usuário alterasse uma opção protegida, ele interromperá a instalação e exibirá uma mensagem. Esses recursos de segurança operam somente quando o programa de instalação é executado em um contexto de segurança privilegiado e tem acesso aos diretórios negados ao usuário. Essa configuração de política é feita para ambientes menos restritivos. Ela pode ser usado para contornar erros em um programa de instalação que impede a instalação do software.  
  [Saiba mais](https://go.microsoft.com/fwlink/?linkid=2067060)  

  **Padrão**: Sim

- **Bloquear as instalações de aplicativo com privilégios elevados**  
...
- **Nível de autenticação**  
  ...
  - *Enviar LM e NTLM – NTLMv2 se negociado*: os clientes usarão a autenticação LM e NTLM e usarão a segurança de sessão NTLMv2 se o servidor for compatível com ela; controladores de domínio aceitam autenticações LM, NTLM e NTLMv2. 
...
Para obter mais informações, confira [2.2.2 FW_PROFILE_TYPOE]( https://docs.microsoft.com/openspecs/windows_protocols/ms-fasp/7704e238-174d-4a5e-b809-5f3787dd8acc) na documentação de Protocolos do Windows.  
...
- **Bloquear o Windows Hello para Empresas**  
  ...
  - Quando definido como *Desabilitar*, os dispositivos provisionam o Windows Hello para Empresas.
  - Quando definido como *Não configurado*, a linha de base não afeta a configuração de política do dispositivo. Isso significa que o status atual do Windows Hello para Empresas em um dispositivo permanece em vigor.

  Você pode desabilitar o Windows Hello para Empresas ao configurar o [registro do Windows](windows-hello.md) ou como parte de um perfil de configuração de dispositivo para [proteção de identidade](identity-protection-configure.md).  

  - **Padrão**: Habilitada

  As configurações a seguir estão disponíveis quando essa configuração é definida como *Desabilitada*.
...
- **Atualizações da barra de status por meio de script da zona do Internet Explorer**
- **Arrastar e soltar ou copiar e colar arquivos na zona do Internet Explorer**  
- **Componentes dependentes de .NET Framework da zona restrita do Internet Explorer**  
- **Não executar antimalware nos controles ActiveX em zona de computador local do Internet Explorer**
- **Suporte à criptografia do Internet Explorer**  
...
- **Aviso automático da zona de Internet do Internet Explorer para downloads de arquivos** > O valor padrão agora é **Desabilitado**. Na versão prévia, isso estava definido como Habilitado.
