...
- **Application Guard**: selecione **Habilitado para o Edge** para ativar esse recurso, que abre sites não confiáveis em um contêiner de navegação virtualizado do Hyper-V. **Não configurado** (padrão) significa que qualquer site (confiáveis e não confiáveis) é aberto no dispositivo.
...
- **Imprimir no navegador virtual**: escolha **Permitir** para que impressoras de rede, locais, PDF e XPS possam imprimir o conteúdo por meio do navegador virtual. **Não configurado** (padrão) desabilita todos os recursos de impressão.
...
- **Verificação da lista de revogação de certificado**: escolha como o dispositivo verifica a lista de revogação de certificado. As opções incluem **Desabilitar verificação da CRL**, **Falhar verificação da CRL somente em certificados revogados** e **Falhar verificação da CRL em qualquer erro encontrado**.
...
- **Enfileiramento de pacotes**: informe como o dimensionamento do software no lado do recebimento será habilitada para o recebimento criptografado e o encaminhamento de texto não criptografado para o cenário de gateway de túnel IPsec. Essa configuração confirma que a ordem de pacote é preservada.
...
- **Regras do Windows Defender Firewall de aplicativo autorizado do repositório local**: escolha **Habilitar** para aplicar as regras de firewall no repositório local de modo que elas sejam reconhecidas e impostas. Quando essa opção está definida como **Não configurado** (padrão), as regras de firewall de aplicativo autorizado no repositório local são ignoradas e não são impostas.
- **Regras do Windows Defender Firewall de porta global do repositório local**: escolha **Habilitar** para aplicar as regras de firewall de porta global no repositório local a serem reconhecidas e impostas. Quando essa opção está definida como **Não configurado** (padrão), as regras de firewall de porta global no repositório local são ignoradas e não são impostas.
- **Regras do Windows Defender Firewall do repositório local**: escolha **Habilitar** para aplicar as regras de firewall no repositório local a serem reconhecidas e impostas. Quando essa opção está definida como **Não configurado** (padrão), as regras de firewall no repositório local são ignoradas e não são impostas.
- **Regras do IPsec do repositório local**: escolha **Habilitar** para aplicar as regras de segurança de conexão no repositório local, independentemente das versões de regra de segurança da conexão ou do esquema. Quando essa opção está definida como **Não configurado** (padrão), as regras de segurança da conexão no armazenamento local são ignoradas e não são impostas, independentemente das versões de regra de segurança da conexão ou do esquema.
...
- **Aviso para outras criptografias de disco**: selecione **Bloquear** para desabilitar o prompt de aviso se outro serviço de criptografia de disco estiver no dispositivo. A opção **Não configurado** (padrão) permite que o aviso seja mostrado.
    - **Permitir que usuários padrão habilitar a criptografia durante o ingresso no Azure AD**: quando você escolhe **permitir**, padrão usuários/não-administradores podem habilitar a criptografia BitLocker quando o usuário está conectado. Essa configuração só se aplica a dispositivos do Azure Active Directory Ingressados (Azure ADJ). **Não configurado** só permite que os Administradores habilitem a criptografia do BitLocker no dispositivo.
...
- **Recuperação de unidade do sistema operacional**: **habilite** essa configuração para controlar como as unidades do sistema operacional protegidas pelo BitLocker são recuperadas quando as informações de inicialização necessárias não estão disponíveis. Quando essa opção está definida como **Não configurado** (padrão), as opções de recuperação padrão são compatíveis com a recuperação do BitLocker. Por padrão, um DRA é permitido, as opções de recuperação são escolhidas pelo usuário, incluindo a senha e a chave de recuperação, e as informações de recuperação não são copiadas em backup no AD DS.
  ...
  - **Salvar informações de recuperação do BitLocker no AD DS**: escolha **Habilitar** para armazenar de informações de recuperação do BitLocker no AAD (Azure Active Directory). Quando a opção é definida como **Não configurado** (padrão), as informações de recuperação não são armazenadas no AAD.
  ...
  - **Armazenar informações de recuperação no AD DS antes de habilitar o BitLocker**: selecione **Exigir** essa configuração para impedir que os usuários ativem o BitLocker, a menos que as informações de recuperação do BitLocker tenham sido armazenadas com êxito no Azure AD (Active Directory). A opção **Não configurado** (padrão) permite que os usuários ativem o BitLocker, mesmo se as informações de recuperação não sejam armazenadas com êxito no Azure AD.
...
- **Recuperação de unidade fixa**: **habilite** essa configuração para controlar como as unidades fixas protegidas pelo BitLocker são recuperadas quando as informações de inicialização necessárias não estão disponíveis. **Não configurado** (padrão) desabilita esse recurso.
  ...
  - **Opções de recuperação no assistente de instalação do BitLocker**: definido como **Bloquear** para que os usuários não possam ver e alterar as opções de recuperação. Quando essa opção está definida como **Não configurado** (padrão), os usuários podem ver e alterar as opções de recuperação ao ativarem o BitLocker.
  - **Salvar informações de recuperação do BitLocker no Azure Active Directory**: escolha **Habilitar** para armazenar de informações de recuperação do BitLocker no Azure AD (Active Directory). Quando essa opção é definida como **Não configurado** (padrão), as informações de recuperação não são armazenadas no Azure AD.
  - **Informações de recuperação do BitLocker armazenadas no Azure Active Directory**: configure quais partes das informações de recuperação do BitLocker são armazenadas no Azure AD. Suas opções:
    - **Fazer backup de pacotes de chaves e senhas de recuperação**
    - **Fazer backup somente de senhas de recuperação**
  - **Armazenar informações de recuperação no Azure Active Directory antes de habilitar o BitLocker**: selecione **Exigir** essa configuração para impedir que os usuários ativem o BitLocker, a menos que as informações de recuperação do BitLocker tenham sido armazenadas com êxito no Azure AD. A opção **Não configurado** (padrão) permite que os usuários ativem o BitLocker, mesmo se as informações de recuperação não sejam armazenadas com êxito no Azure AD.
...
> [!IMPORTANT]
> Para permitir que a instalação correta e a execução de aplicativos de LOB Win32, configurações de antimalware devem excluir os seguintes diretórios que estão sendo verificados:<p>
> **No X64 máquinas cliente**:<br>
> *C:\Program arquivos (x86) \Microsoft Intune gerenciamento Extension\Content*<br>
> *C:\windows\IMECache*
>  
> **Em X86 máquinas cliente**:<br>
> *C:\Program Files\Microsoft Intune gerenciamento Extension\Content*<br>
> *C:\windows\IMECache*
...
- **Proteção de rede**: protege as conexões de saída de qualquer aplicativo para endereços IP de baixa reputação ou domínios. A intenção é proteger os usuários finais de aplicativos com acesso de phishing scams, hospedagem de exploração de sites e conteúdo mal-intencionado na Internet. Ele também impede que os navegadores de terceiros se conectar a sites perigosos.

  Suas opções:

  - **Não configurado** (padrão) desabilita esse recurso. Usuários e aplicativos não são impedidos de se conectar a domínios perigosos. Os administradores não podem ver essa atividade na Central de segurança do Windows Defender.
  - **Habilitar** ativa a proteção de rede e bloqueia os usuários e aplicativos de se conectar a domínios perigosos. Os administradores podem ver essa atividade na Central de segurança do Windows Defender.
  - **Somente auditoria**: usuários e aplicativos não são impedidos de se conectar a domínios perigosos. Os administradores podem ver essa atividade na Central de segurança do Windows Defender.
...
- **Fazer logon remoto sem senha**: a opção **Bloquear** permite somente a conexão de contas locais com senhas em branco usando o teclado do dispositivo. **Não configurado** (padrão) permite a conexão de contas locais com senhas em branco de locais que não sejam o dispositivo físico.
...
- **Minutos de inatividade da tela de bloqueio até a proteção de tela ser ativada**: insira o máximo de minutos de inatividade na tela de entrada da área de trabalho interativa até que a proteção de tela seja executada.
...
- **Restringir conexões RPC remotas para SAM**: definido como **permitir** para negar a usuários e grupos de fazer chamadas RPC remotas para segurança contas Manager (SAM), que armazena as contas de usuário e senhas. **Permitir** também permite a você alterar a cadeia de caracteres de idioma de definição de descritor de segurança (SDDL) padrão para permitir ou negar a usuários e grupos de fazer essas chamadas remotas explicitamente. **Não configurado** (padrão) usa o descritor de segurança padrão e pode permitir que usuários e grupos para fazer chamadas RPC remotas para SAM.
...
- **Integridade UIA sem local seguro**: quando essa opção está definida como **Bloquear**, os aplicativos em uma localização segura no sistema de arquivos são executados somente com a integridade de UIAccess. A opção **Não configurado** (padrão) permite que os aplicativos sejam executados com a integridade de UIAccess, mesmo se eles não estiverem em um local seguro no sistema de arquivos.
- **Virtualizar falhas de gravação de arquivo e registro para locais por usuário**: quando definido como **Enabled**, aplicativos que gravam dados em localizações protegidas apresentam falhas. Quando definido para **Não configurado** (padrão), falhas de gravação de aplicativos são direcionadas em tempo de execução a localizações de usuário definidas tanto para o sistema de arquivos quanto para o Registro.
...
- **Prompt com privilégios elevados para instalações de aplicativo**: quando essa opção é definida como **Habilitado**, os pacotes de instalação de aplicativo não são detectados nem solicitados para elevação. Quando essa opção está definida como **Não configurado** (padrão), o usuário é solicitado a fornecer um nome de usuário administrativo e uma senha quando um pacote de instalação de aplicativo exige privilégios elevados.
...
- **Executar todos os administradores no Modo de Aprovação de Administrador**: defina essa opção como **Habilitado** para desabilitar o Modo de Aprovação de Administrador e todas as configurações de política UAC relacionadas. A opção **Não configurado** (padrão) habilita o Modo de Aprovação de Administrador.
