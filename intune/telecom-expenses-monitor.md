...
description: Integre o Intune ao serviço de gerenciamento de despesas de telecomunicações da Saaswedo.
keywords: Saaswedo
...
O Intune permite que você gerencie as despesas de telecomunicações decorrentes da utilização de dados em dispositivos móveis corporativos. Para habilitar essa funcionalidade, o Intune integrou a solução de [gerenciamento de despesas de telecomunicações Datalert](http://datalert.biz/get-started) do desenvolvedor de software de terceiros Saaswedo. O Datalert é um software de gerenciamento de despesas de telecomunicações em tempo real que permite gerenciar o uso de dados de telecomunicações. Ele ajuda a evitar excedentes caros e inesperados de dados e roaming para seus dispositivos gerenciados pelo Intune.

A integração do Intune com o Datalert permite definir, monitorar e impor limites de uso de dados de roaming e doméstico centralmente. Alertas automatizados são disparados quando os limites excedem os valores definidos. Você pode configurar o serviço para aplicar diferentes ações a indivíduos ou grupos de usuários finais, incluindo desabilitar roaming ou exceder o limite. Relatórios que fornecem o uso de dados e informações de monitoramento estão disponíveis no console de gerenciamento Datalert.
...
3. Selecione **Desbloquear** na parte inferior da página, o que permite que você modifique as configurações na página.

4. Na seção **Conexão do Datalert/Intune**, escolha **Microsoft Intune** para **servidor MDM**.    

5. Para **Domínio do Azure AD**, insira sua ID de locatário do Azure e, em seguida, selecione o botão **Conexão**.

    Quando você seleciona **Conexão**, o serviço Datalert verifica com o Intune se não há conexões Datalert preexistentes com o Intune. Depois de alguns segundos, uma página de logon do Microsoft aparecerá, seguida da autenticação do Datalert Azure.

6. Na página de autenticação da Microsoft, selecione **Aceitar**. Você será redirecionado para uma página de **agradecimento** do Datalert, que se fecha depois de alguns segundos. O Datalert valida a conexão e exibe uma marca de seleção verde ao lado de uma lista de itens validados. Se a validação falhar, você verá uma mensagem em vermelho e deverá contatar o Suporte da Datalert para obter ajuda.
   ...
   ![Página do Datalert mostrando a conexão bem-sucedida](./media/tem-datalert-connection.png)

7. Na seção **Consentimento ADAL/Aplicativo Datalert**, defina a opção como **Ligada**. Na página de autenticação da Microsoft, selecione **Aceitar**. Você será redirecionado para uma página de **agradecimento** do Datalert, que se fecha depois de alguns segundos. O Datalert valida a conexão e exibe uma marca de seleção verde ao lado de uma lista de itens validados. Se a validação falhar, você verá uma mensagem em vermelho e deverá contatar o Suporte da Datalert para obter ajuda.    

    A captura de tela a seguir mostra as marcas de seleção verde que você pode esperar encontrar quando a conexão for bem-sucedida.

   ![Página do Datalert mostrando a conexão bem-sucedida](./media/tem-datalert-adal-consent.png)

8. Na seção **Gerenciamento de Perfis de MDM (opcional)** seção, defina a opção como **Ligada** para permitir que o Datalert leia os perfis disponíveis no Intune para ajudá-lo configurar políticas. Na página de autenticação da Microsoft, selecione **Aceitar**. Você será redirecionado para uma página de **agradecimento** do Datalert, que se fecha depois de alguns segundos. O Datalert valida a conexão e exibe uma marca de seleção verde ao lado de uma lista de itens validados. Se a validação falhar, você verá uma mensagem em vermelho e deverá contatar o Suporte da Datalert para obter ajuda.    

    A captura de tela a seguir mostra as marcas de seleção verde que você pode esperar encontrar quando a conexão for bem-sucedida.

   ![Página do Datalert mostrando a conexão bem-sucedida](./media/tem-datalert-mdm-profiles.png)
...
4. No painel **Configuração do dispositivo**, escolha **Instalação** > **Gerenciamento de Despesas de Telecomunicações**.

   Procure o status de conexão **Ativo** na parte superior da página.

   ![Página do Intune mostrando o status da conexão do Datalert Ativa](./media/tem-azure-portal-enable-service.png)
...
Para garantir que somente o uso de dada de linhas corporativas seja coletado, é necessário fazer duas coisas:
- criar categorias de dispositivos no Intune
- afetar o aplicativo Datalert apenas em smartphones corporativos.
...
Crie pelo menos duas categorias de dispositivos (por exemplo, Corporativo e Pessoal), dependendo das suas necessidades da sua organização. Em seguida, crie grupos de dispositivos dinâmicos para cada categoria. Você pode criar mais categorias para sua organização, conforme necessário.
...
Essas categorias serão mostradas aos usuários durante o registro. Dependendo de qual categoria o usuário escolher, o dispositivo registrado será movido para o grupo de dispositivo correspondente. Para obter etapas sobre como criar categorias de dispositivos, consulte [Mapear dispositivos para grupos](device-group-mapping.md).
  ...
  ![Captura de tela do painel Adicionar uma política](./media/tem-dynamic-membership-rules.png)
...
1. No painel **Intune** do [portal do Azure](https://portal.azure.com), escolha **Aplicativos clientes**.

2. No painel **Aplicativos clientes**, escolha **Gerenciar** > **Aplicativos**.
...
6. Selecione o aplicativo **Datalert** e escolha **Selecionar**.

   ![Captura de tela do painel Adicionar uma política](./media/tem-select-app-from-apple-app-store.png)

7. Conclua as etapas restantes para criar um aplicativo para iOS.

   ![Captura de tela do painel Adicionar uma política](./media/tem-steps-to-create-the-app.png)
...
1. No painel **Aplicativos Cliente – Aplicativos**, selecione o aplicativo Datalert para iOS que você criou na etapa anterior.

2. No painel **Aplicativos**, escolha **Gerenciar** > **Atribuições**.

3. Escolha **Adicionar grupo** e siga as etapas para selecionar o grupo de dispositivos corporativos.
...
4. Escolha se deseja tornar a instalação do aplicativo obrigatória ou opcional para o grupo. A captura de tela de exemplo a seguir mostra a instalação conforme necessário, o que significa que os usuários devem instalar o aplicativo Datalert depois de registrar seu dispositivo.
   ...
   ![Captura de tela do painel Adicionar uma política](./media/tem-assign-datalert-app-to-device-group.png)

### <a name="step-4-add-corporate-paid-phone-lines-to-the-datalert-console"></a>Etapa 4: Adicionar linhas telefônicas corporativas pagas ao console do Datalert

Agora você configurou os serviços Intune e Datalert para se comunicar entre si. Você agora precisa adicionar linhas telefônicas pagas corporativas ao console do Datalert e definir limites e ações para qualquer violação de uso de dados celulares ou roaming. É possível adicionar manualmente linhas telefônicas corporativas pagas ao console do Datalert ou adicioná-las automaticamente depois que o dispositivo estiver registrado no Intune.

Para configurar esses itens, vá para [a página Configuração do Datalert para Microsoft Intune](http://www.datalert.fr/microsoft-intune/intune-setup) (http://www.datalert.fr/microsoft-intune/intune-setup), e siga as etapas no assistente de instalação na guia **Configurações**.

  ![Captura de tela do painel Adicionar uma política](./media/tem-add-phone-lines-to-datalert-console.png)
...
Para a experiência de registro de cliente, consulte o seguinte:
...
1. No painel **Gerenciamento de Despesas de Telecomunicações** no Portal do Azure, selecione **Desabilitar**.
