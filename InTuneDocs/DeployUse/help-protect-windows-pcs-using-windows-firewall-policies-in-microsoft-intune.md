...
## Usar as políticas do Intune para gerenciar Firewall do Windows
...
## Especificar as configurações de política do Firewall do Windows
...
- Conectados a uma rede privada (confiável) (como uma rede doméstica)
...
### Configurar exceções predefinidas
...
|**BranchCache - Descoberta de Mesmo Nível**<br>(Windows 7 ou posterior)|Permite que os clientes BranchCache usem o protocolo de descoberta de serviços Web (WS-Discovery) para pesquisar a disponibilidade do conteúdo na sub-rede local.|
|**Serviço de Cache de Mesmo Nível de BITS**|Permite que os clientes usem BITS (Background Intelligent Transfer Service) para encontrar e compartilhar arquivos que estão armazenados no cache BITS em clientes na mesma sub-rede. Essa configuração usa serviços Web em dispositivos (WSDAPI) e Chamada de procedimento remoto (RPC).|
...
|**Extensores do Media Center**|Permite que os Media Center Extenders se comuniquem com computadores executando o Windows Media Center. Essa configuração usa Simple Service Discovery Protocol (SSDP) e qWave.|
...
|**SSTP (Secure Socket Tunneling Protocol)**|Permite conexões de entrada VPN a computadores gerenciados com SSTP (Secure Socket Tunneling Protocol). Essa configuração usa HTTPS.|
...
|**Windows Remote Management**<br>(Windows 8 ou posterior)|Permite o gerenciamento remoto de computadores gerenciados com o WS-Management, um protocolo baseado em serviços Web para o gerenciamento remoto de sistemas operacionais e dispositivos.|
