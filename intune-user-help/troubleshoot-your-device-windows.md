---
title: Corrigir problemas comuns com o dispositivo Windows | Microsoft Docs
description: Descubra maneiras de resolver problemas comuns com o dispositivo Windows.
keywords: ''
author: lenewsad
ms.author: lanewsad
manager: dougeby
ms.date: 10/01/2018
ms.topic: article
ms.prod: ''
ms.service: microsoft-intune
ms.subservice: end-user
ms.technology: ''
ms.assetid: 48644443-ff50-4af1-9c23-443e6e5c3b47
searchScope:
- User help
ROBOTS: ''
ms.reviewer: priyar
ms.suite: ems
ms.collection: M365-identity-device-management
ms.openlocfilehash: 29a79e109f40a06930f988156e7621a4b55c05f4
ms.sourcegitcommit: ebf72b038219904d6e7d20024b107f4aa68f57e6
ms.translationtype: MTE75
ms.contentlocale: pt-BR
ms.lasthandoff: 12/05/2019
ms.locfileid: "72502020"
---
# <a name="fix-common-issues-with-your-windows-device"></a>Corrigir problemas comuns com o dispositivo Windows

Às vezes, podem ocorrer problemas com seu dispositivo Windows, que poderão ser corrigidos pela solução de problemas. Alguns dos problemas comuns que já vimos e que você pode primeiro tentar corrigir sozinho estão listados abaixo. Para os problemas mais complexos, é recomendável contatar o suporte de sua empresa por meio de suas informações de contato no [site do Portal da Empresa](https://go.microsoft.com/fwlink/?linkid=2010980).

- [Não consigo registrar meu dispositivo Windows 10](troubleshoot-your-windows-10-device-windows.md)
- [Preciso atualizar meu dispositivo Windows](you-need-to-update-your-windows-device.md)
- [Preciso enviar logs para o suporte de minha empresa no aplicativo do Portal da Empresa](send-logs-to-your-it-admin-cp-windows.md)
- [Preciso enviar logs para o suporte de minha empresa no aplicativo de Configurações](send-logs-to-your-it-admin-settings-windows.md)
- [Ainda não há suporte para minha versão do Windows](your-windows-version-isnt-yet-supported.md)
- [Preciso habilitar a criptografia do Windows](you-need-to-enable-windows-encryption.md)
- [Meu dispositivo precisa de software antimalware](your-device-needs-antimalware-software.md)
- [Preciso habilitar a Inicialização Segura](you-need-to-enable-secure-boot-windows.md)
- [Preciso habilitar a Integridade de Código](you-need-to-enable-code-integrity.md)
- [É necessário habilitar o Firewall](you-need-to-enable-defender-firewall-windows.md)
- [É necessário habilitar o UAC](you-need-to-enable-uac-windows.md)
- [É necessário atualizar o Windows](you-need-to-update-os-build-version-windows.md)
- [É necessário ativar o Windows Defender](turn-on-defender-windows.md)
